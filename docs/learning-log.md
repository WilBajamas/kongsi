...
- **Failed syncs are invisible** — [ADR-024](adr/ADR-024-surface-failed-syncs.md) is Accepted but **not built**. Needs: `watchFailed()` on `OutboxRepository`, a UI surface, a retry action, and removal of the `_maxAttempts` check from the drain. Discard is deliberately out of scope until rollback exists.
